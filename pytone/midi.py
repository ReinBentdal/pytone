...
class Note:
    ...
    @property
    def pressed(self):
        return self.state == State.NOTE_ON
    ...
    def copy_with(
        self, note: int = None, velocity: int = None, state: State = None, channel: int = None
    ):
        return Note(
    ...
    def __str__(self):
        return (
            f"Note: {self.note}, velocity: {self.velocity}, channel: {self.channel}, {self.state}"
        )
...
class Module:
    ...
    def reset(self):
        """Resets the sketch to its initial state"""
        pass
