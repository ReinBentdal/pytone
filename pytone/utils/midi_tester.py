...
class Tester:
    def __init__(
        ...
        samples_dir=None,
        bpm=120,
        out=TesterOutput.AUDIO,
        keyboard_layout=keyboard_layout_no,
    ):
        self.out = out
        self.module = module
        self.bpm = bpm
        ...
        self.key_to_midi = {
            char: num + keyboard_layout["base"] for char, num in keyboard_layout["keys"].items()
        }
        self.running = True

        if samples_dir is None:
            # default to SalamanderGrandPianoV3
            samples_dir = "soundbanks/SalamanderGrandPianoV3/samples"
            samples_dir = os.path.join(os.path.dirname(__file__), "..", "..", samples_dir)

        # assert samples_dir and 88 keys
        assert os.path.isdir(samples_dir)
        assert len(glob.glob(os.path.join(samples_dir, "*.wav"))) == 88
        ...
        if out == TesterOutput.MIDI:
            ...
            self.audio_playing = {}
            self.audio_notes = {
                int(fname[4:7]): sa.WaveObject.from_wave_file(f"{samples_dir}/{fname}")
                for fname in os.listdir(f"{samples_dir}/")
    ...
    def send_midi(self, note):
        ...
        msg = mido.Message(
            "note_on" if note.pressed else "note_off",
            note=note.note,
